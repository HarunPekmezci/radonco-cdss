...
export type UterusSetting = 'newly-diagnosed' | 'postoperative' | 'recurrent' | 'metastatic';
export type UterusHistology = 'endometrioid' | 'serous' | 'clear-cell' | 'carcinosarcoma' | 'sarcoma' | 'other' | 'unknown';
export type UterusFIGOStage = 'IA' | 'IB' | 'II' | 'IIIA' | 'IIIB' | 'IIIC1' | 'IIIC2' | 'IVA' | 'IVB' | 'unknown';
export type UterusMolecularClass = 'POLE-mutated' | 'MMRd' | 'NSMP' | 'p53-abnormal' | 'HER2-positive' | 'ER-positive' | 'none' | 'unknown';
export type UterusRisk = 'low' | 'intermediate' | 'high-intermediate' | 'high' | 'unknown';

export interface UterusInput extends ClinicalCaseInput {
  organSystem: 'gynecology';
  disease: 'uterine-cancer';
  setting: UterusSetting;
  stage?: TNMStage;
  figoStage?: UterusFIGOStage;
  histology?: UterusHistology;
  molecularClass?: UterusMolecularClass;
  riskCategory?: UterusRisk;
...
const ESTRO: GuidelineReference = {
  organization: 'ESTRO',
  title: 'ESGO-ESTRO-ESP endometrial cancer risk classification and adjuvant therapy guidance',
...
const TRIALS: GuidelineReference[] = [
  ...
  { organization: 'other', title: 'RUBY: dostarlimab plus carboplatin/paclitaxel in primary advanced or recurrent endometrial cancer', url: 'https://doi.org/10.1056/NEJMoa2216334', evidenceLevel: '1' },
  { organization: 'other', title: 'DUO-E: durvalumab-based first-line treatment in advanced endometrial cancer', url: 'https://doi.org/10.1056/NEJMoa2307909', evidenceLevel: '1' },
];

export const UTERUS_INPUT_JSON_SCHEMA: JsonSchema = {
  $schema: 'http://json-schema.org/draft-07/schema#',
  $id: 'https://radonco.local/schema/uterus-input.json',
  title: 'Uterine cancer clinical case input',
  type: 'object',
  required: ['organSystem', 'disease', 'setting'],
  properties: {
    organSystem: { type: 'string', enum: ['gynecology'] },
    disease: { type: 'string', enum: ['uterine-cancer'] },
    ...
    histology: { type: 'string', enum: ['endometrioid', 'serous', 'clear-cell', 'carcinosarcoma', 'sarcoma', 'other', 'unknown'] },
...
export const UTERUS_OUTPUT_JSON_SCHEMA: JsonSchema = {
  $schema: 'http://json-schema.org/draft-07/schema#',
  $id: 'https://radonco.local/schema/uterus-result.json',
  title: 'Uterine cancer CDSS result',
  type: 'object',
  required: ['schemaVersion', 'engineId', 'engineVersion', 'organSystem', 'rtIndication', 'intent', 'summary', 'recommendations'],
  properties: {
    schemaVersion: { type: 'string', enum: ['1.0'] },
    engineId: { type: 'string', enum: ['gynecology.uterus'] },
...
const uterusTargets = (dose: Fractionation, postoperative = false): TargetVolume[] => [
  { name: 'GTV', description: postoperative ? 'Vajinal cuff/cerrahi yatak ve rezidüel nüks; MRI/PET-CT ile.' : 'Uterin primer veya makroskopik pelvik/paraaortik nodal hastalık.', dose, margin: 'Görüntülenebilir hastalık' },
  { name: 'CTV', description: postoperative ? 'Vajinal cuff, paravaginal doku ve patolojik risk alanları.' : 'Uterus/serviks, parametrial risk alanı ve histoloji/evreye göre pelvik ± paraaortik nodlar.', dose, margin: 'MRI, cerrahi klipler ve anatomik bariyerler dikkate alınır' },
  { name: 'PTV', description: 'Mesane ve rektum dolumu, uterus hareketi ve günlük IGRT belirsizliği.', dose, margin: 'CBCT/IGRT ile yaklaşık 5-10 mm; adaptif planlama seçilebilir' },
];

const uterusOars: OARConstraint[] = [
...
export class UterusDecisionEngine extends BaseDecisionEngine<UterusInput> {
  public readonly id = 'gynecology.uterus';
  public readonly version = '1.0.0';
  public readonly inputSchema = UTERUS_INPUT_JSON_SCHEMA;
  public readonly outputSchema = UTERUS_OUTPUT_JSON_SCHEMA;

  public constructor() {
    super('1.0.0');
  }

  protected evaluateCase(input: UterusInput): CDSSResult {
    ...
    if (input.molecularClass === 'MMRd' || input.molecularClass === 'p53-abnormal') warnings.push('Moleküler sınıf adjuvan sistemik tedavi, RT yoğunluğu ve klinik çalışma seçimini etkiler; entegre moleküler rapor doğrulanmalıdır.');
    ...
    if (input.setting === 'newly-diagnosed' || input.setting === 'postoperative') {
      ...
      recommendations.push({
        ...
        fractionation,
        targetVolumes: fractionation ? uterusTargets(fractionation, postoperative) : undefined,
        oarConstraints: fractionation ? uterusOars : undefined,
        systemicTherapy: systemicTherapy.length ? systemicTherapy : undefined,
        rationale: [
          'Temel cerrahi yaklaşım total histerektomi ve uygun nodal evrelemedir; R0 hedeflenir, R1/R2 ve pozitif nodlar adjuvan tedaviyi değiştirir.',
          ...
          'PORTEC-3, yüksek riskli histoloji ve evrede kombine kemoradyoterapi yaklaşımının rolünü destekler; moleküler sınıf modern risk değerlendirmesine eklenir.',
      ...
      if (intermediateRisk && !highRisk) {
        recommendations[0].fractionation = cuffDose;
        recommendations[0].targetVolumes = uterusTargets(cuffDose, true);
      }
      ...
      recommendations.push({
        id: 'recurrent-uterine-cancer',
        label: 'Lokal uterus kanseri nüksünde salvage cerrahi veya seçilmiş RT/brakiterapi',
        indication: input.priorPelvicRT ? 'consider' : 'indicated',
        intent: 'salvage',
        fractionation: input.priorPelvicRT ? undefined : salvageDose,
        targetVolumes: input.priorPelvicRT ? undefined : uterusTargets(salvageDose, true),
        oarConstraints: input.priorPelvicRT ? undefined : uterusOars,
        systemicTherapy: [systemic('palliative', 'İleri/nüks endometrium kanserinde sistemik tedavi', ['carboplatin + paclitaxel ± dostarlimab/pembrolizumab'], 'MMR/MSI, p53, HER2 ve önceki tedaviye göre')],
        rationale: [
      ...
      const therapy: SystemicTherapyRecommendation[] = [
        systemic('palliative', 'İleri uterus kanserinde karboplatin/paklitaksel ± immünoterapi', ['carboplatin', 'paclitaxel', 'dostarlimab veya pembrolizumab'], 'MMR/MSI, p53, performans ve organ fonksiyonuna göre'),
      ];
      ...
      recommendations.push({
        id: 'metastatic-uterine-cancer',
        label: 'Metastatik uterus kanserinde moleküler profile göre sistemik tedavi ve semptomatik RT',
        indication: 'conditional',
        intent,
        fractionation: palliativeDose,
        targetVolumes: uterusTargets(palliativeDose),
        oarConstraints: uterusOars,
        systemicTherapy: therapy,
        rationale: [
          'RUBY ve DUO-E, ileri veya nüks endometrium kanserinde kemoterapiye immünoterapi eklenmesini destekler.',
    ...
    rationale.push('Uterus kanseri kararı; jinekolojik onkoloji, radyasyon/medikal onkoloji, patoloji ve radyoloji konseyinde FIGO/TNM evresi, histoloji, moleküler sınıf, cerrahi patoloji ve hasta hedefleriyle verilmelidir.');
    return {
      schemaVersion: '1.0',
      engineId: this.id,
  ...
  private summary(input: UterusInput): string {
    if (input.setting === 'newly-diagnosed' || input.setting === 'postoperative') return 'Yeni tanı/postoperatif uterus kanseri: cerrahi patoloji, FIGO, moleküler sınıf ve risk grubuna göre sürveyans, brakiterapi veya pelvik KRT.';
    if (input.setting === 'recurrent') return 'Nüks uterus kanseri: salvage cerrahi/RT ve moleküler profile göre sistemik tedavi.';
    return 'Metastatik uterus kanseri: karboplatin/paklitaksel temelli immüno-kemoterapi ve semptom odaklı RT.';
  }
}

export const uterusEngine = new UterusDecisionEngine();

export default uterusEngine;
