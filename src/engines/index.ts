...
// ============================================================================
// GÜS, GİS, SARKOM, CİLT VE DİĞER TÜMÖR MOTORLARI (TAM KAPSAMLI)
// ============================================================================
...
export function runProstateEngine(data: any): EngineOutput {
  ...
  const chhip: DoseRegimen = { categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Hipofraksiyone', regimenName: 'CHHiP (60 Gy / 20 fx)', totalDoseGy: 60, fractionCount: 20, dosePerFractionGy: 3, alphaBeta: 1.5, bedGy: 180, eqd2Gy: 77.1, targetVolumeCTV: 'Prostat ± SV', volumeUnionHierarchy: 'PTV = CTV + 5 mm', gtvToCtvMargin: '0 mm', ctvToPtvMargin: 'Post 4mm, Diğer 5-6mm', clinicalIndication: 'Standart lokalize', preferredBadge: !isHigh };
  const sbrt: DoseRegimen = { categoryRank: '3_ULTRA_HIPOFRAKSIYONE_SBRT', modalityDisplay: 'SBRT', regimenName: 'PACE-B (36.25 Gy / 5 fx)', totalDoseGy: 36.25, fractionCount: 5, dosePerFractionGy: 7.25, alphaBeta: 1.5, bedGy: 211.5, eqd2Gy: 90.6, targetVolumeCTV: 'Prostat', volumeUnionHierarchy: 'PTV = CTV + 3-4 mm', gtvToCtvMargin: '0 mm', ctvToPtvMargin: '3-4 mm', clinicalIndication: 'Ablatif standart', preferredBadge: !isHigh && !isUnfav };
  const sib: DoseRegimen = { categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Pelvik SIB', regimenName: 'SIB (70/56/46 Gy)', totalDoseGy: 70, fractionCount: 28, dosePerFractionGy: 2.5, alphaBeta: 1.5, bedGy: 186.7, eqd2Gy: 80, targetVolumeCTV: 'Prostat + SV + Pelvik LN', volumeUnionHierarchy: 'PTV_46 ∪ PTV_56 ∪ PTV_70', gtvToCtvMargin: 'Pelvik LN + 7mm', ctvToPtvMargin: '5 mm', clinicalIndication: 'Yüksek risk nodal kapsama', preferredBadge: isHigh };

  const regimens = data.hasM1 ? [{ ...chhip, totalDoseGy: 55, dosePerFractionGy: 2.75, regimenName: 'STAMPEDE (55 Gy)', bedGy: 155.8, eqd2Gy: 66.8, preferredBadge: true }] : isHigh ? [sib, chhip] : isUnfav ? (roach >= 15 ? [sib, chhip] : [chhip, sbrt]) : [chhip, sbrt];
  const strategy = data.lifeExpectancyLess10 && !data.hasM1 && !isHigh ? 'Gözlem (Observation). Yaşam beklentisi kısa olan lokalize hastalıkta küratif RT önerilmez.' : isHigh ? 'Pelvik SIB + Uzun Dönem ADT.' : isUnfav ? 'Prostat RT + Kısa Dönem ADT.' : 'Aktif İzlem veya Monoterapi RT.';

  return { ajccStage: stage, stageSummary: data.hasM1 ? 'Metastatik' : isHigh ? 'Yüksek Risk' : isUnfav ? 'Uygunsuz Orta Risk' : 'Düşük/Uygun Orta Risk', color: isHigh ? 'bg-rose-50 text-rose-900 border-rose-300' : isUnfav ? 'bg-amber-50 text-amber-800 border-amber-300' : 'bg-emerald-50 text-emerald-800 border-emerald-300', strategy, systemic: isHigh ? 'Uzun ADT ± Abirateron.' : isUnfav ? 'Kısa ADT.' : 'Gerekmez.', roachRisk: roach, coreImpact: `Gleason ${p1}+${p2}=${sum}. ${data.lifeExpectancyLess10 ? 'Kısa yaşam beklentisi.' : ''}`, regimens: data.lifeExpectancyLess10 && !data.hasM1 && !isHigh ? [] : regimens, url: 'https://www.nccn.org', ref: 'NCCN Prostate' };
}
...
export function runBladderEngine(data: any): EngineOutput {
  ...
  return { ajccStage: data.hasM1 ? 'Evre IVB' : data.nodalStatus !== 'cN0' ? 'Evre III' : 'Evre II', stageSummary: data.hasM1 ? 'Metastatik' : data.poorBladderCapacity ? 'Sistektomi Zorunlu' : isIdeal ? 'Trimodalite İdeal Adayı' : 'Yüksek Risk', color: data.poorBladderCapacity ? 'bg-rose-100 text-rose-950 font-bold border-rose-500' : data.hasM1 ? 'bg-purple-50 text-purple-900 border-purple-300' : isIdeal ? 'bg-cyan-50 text-cyan-900 border-cyan-300' : 'bg-amber-50 text-amber-800 border-amber-300', strategy, systemic: 'Sisplatin veya 5-FU/MMC.', coreImpact: data.smokingHistory > 0 ? `Sigara: ${data.smokingHistory} paket/yıl.` : 'Sigara öyküsü yok.', regimens: data.poorBladderCapacity ? [] : regimens, url: 'https://www.nccn.org', ref: 'BC2001 Trial' };
}

// --- BÖBREK (RCC) DETAYLI MOTOR ---
export function runKidneyRccEngine(data: any): EngineOutput {
  ...
  return { 
    ajccStage: stage, 
    stageSummary: data.isSolitaryKidney || data.lowGfr ? 'Nefron Koruma Zorunlu' : 'İnoperabl Primer RCC', 
    color: data.lowGfr ? 'bg-orange-100 text-orange-900 border-orange-400' : 'bg-blue-50 text-blue-900 border-blue-300', 
    strategy: data.lowGfr || data.isSolitaryKidney ? 'Düşük GFR / Soliter Böbrek: Parsiyel nefrektomi veya nefron koruyucu ablatif SBRT.' : 'Ablatif Renal SBRT.', 
    systemic: data.hasM1 ? 'TKI (Kabozantinib) + İmmünoterapi (Pembrolizumab).' : 'Gerekmez.', 
    coreImpact: data.isSolitaryKidney ? 'Soliter böbrek vakası: V10-V15 doz kısıtlamalarına dikkat edilmeli.' : 'Standart RCC yönetimi.',
    regimens, 
    url: 'https://www.nccn.org', 
    ref: 'I収RT / NCCN Kidney v1.2026' 
  };
}
...
export function runRectumEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = data.hasM1 ? [{ categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Palyatif', regimenName: 'Palyatif (30 Gy / 10 fx)', totalDoseGy: 30, fractionCount: 10, dosePerFractionGy: 3, alphaBeta: 10, bedGy: 39, eqd2Gy: 32.5, targetVolumeCTV: 'Tümör', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: '10mm', ctvToPtvMargin: '5mm', clinicalIndication: 'Semptomatik lokal kontrol', preferredBadge: true }] : isHigh ? [{ categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Kısa Kurs', regimenName: 'RAPIDO (25 Gy / 5 fx)', totalDoseGy: 25, fractionCount: 5, dosePerFractionGy: 5, alphaBeta: 10, bedGy: 37.5, eqd2Gy: 31.3, targetVolumeCTV: 'Mezorektum', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: 'Mezorektum', ctvToPtvMargin: '5mm', clinicalIndication: 'TNT Adayı', preferredBadge: true }] : [{ categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Uzun Kurs', regimenName: 'KRT (50.4 Gy)', totalDoseGy: 50.4, fractionCount: 28, dosePerFractionGy: 1.8, alphaBeta: 10, bedGy: 59.5, eqd2Gy: 49.6, targetVolumeCTV: 'Mesorektum', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: '1-2cm', ctvToPtvMargin: '5mm', clinicalIndication: 'Standart', preferredBadge: true }];
...
// --- PANKREAS SBRT MOTORU ---
export function runPancreasEngine(data: any): EngineOutput {
  const isBorderline = data.resectability === 'BORDERLINE_RESECTABLE';
  const regimens: DoseRegimen[] = [
    { categoryRank: '3_ULTRA_HIPOFRAKSIYONE_SBRT', modalityDisplay: 'Pankreas SBRT', regimenName: 'SBRT (33-40 Gy / 5 fx)', totalDoseGy: 40.0, fractionCount: 5, dosePerFractionGy: 8.0, alphaBeta: 10.0, bedGy: 72.0, eqd2Gy: 60.0, targetVolumeCTV: 'Pankreas Kitle + Damar Teması (GTV=CTV)', volumeUnionHierarchy: 'PTV = ITV + 3 mm', gtvToCtvMargin: '0 mm', ctvToPtvMargin: '3 mm', clinicalIndication: 'FOLFIRINOX indüksiyonu sonrası konsolidasyon SBRT', preferredBadge: true }
  ];
  return { 
    ajccStage: data.hasM1 ? 'Evre IV' : isBorderline ? 'Borderline Rezekabl' : 'Lokal İleri (LAPC)', 
    stageSummary: isBorderline ? 'Sınırda Rezekabl Pankreas' : 'İnoperabl Lokal İleri Pankreas', 
    color: 'bg-amber-50 text-amber-900 border-amber-300', 
    strategy: 'İndüksiyon kemoterapisi (FOLFIRINOX) ardından yanıt alan olgularda ablatif SBRT veya KRT.', 
    systemic: 'FOLFIRINOX veya Gemcitabin + Nab-Paclitaxel.', 
    coreImpact: 'Pankreas SBRT planlamasında duodenum ve mide PRV sınırlarına mutlak uyulmalıdır.',
    regimens, 
    url: 'https://www.nccn.org', 
    ref: 'NCCN Pancreatic Adenocarcinoma v1.2026' 
  };
}

// --- KARACİĞER (HCC / MET) SBRT MOTORU ---
export function runLiverEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = isChildA ? [
    { categoryRank: '3_ULTRA_HIPOFRAKSIYONE_SBRT', modalityDisplay: 'Karaciğer SBRT', regimenName: 'SBRT (50 Gy / 5 fx)', totalDoseGy: 50.0, fractionCount: 5, dosePerFractionGy: 10.0, alphaBeta: 10.0, bedGy: 100.0, eqd2Gy: 83.3, targetVolumeCTV: 'Karaciğer Lezyonu (GTV=CTV)', volumeUnionHierarchy: 'PTV = ITV + 5 mm', gtvToCtvMargin: '0 mm', ctvToPtvMargin: '5 mm (ITV ile)', clinicalIndication: 'Child-Pugh A siroz veya oligometastazda ablatif yüksek BED', preferredBadge: true }
  ] : [];

  let strategy = 'Ablatif SBRT ile yüksek lokal kontrol.';
  if (!isChildA) strategy = '⚠️ Child-Pugh B/C Siroz: Karaciğer SBRT/RT karaciğer yetmezliği (RILD) riski nedeniyle kontrendikedir.';

  return { 
    ajccStage: data.hasM1 ? 'Evre IV / Metastatik' : 'HCC / Primer Karaciğer', 
    stageSummary: isChildA ? 'Child-Pugh A Karaciğer SBRT Adayı' : 'Child-Pugh B/C (RT Kontrendike)', 
    color: !isChildA ? 'bg-rose-100 text-rose-950 font-bold border-rose-500' : 'bg-emerald-50 text-emerald-900 border-emerald-300', 
    strategy, 
    systemic: 'Atezolizumab + Bevacizumab (HCC için) veya sistemik kemoterapi.', 
    coreImpact: 'Karaciğer rezervi (Child-Pugh) ve normal karaciğer ortalama doz/V20 sınırları kritik öneme sahiptir.',
    regimens: !isChildA ? [] : regimens, 
    url: 'https://www.nccn.org', 
    ref: 'ASTRO Liver SBRT Guidelines' 
  };
}

export function runLungNsclcEngine(data: any): EngineOutput {
  const isN2 = data.nodalStatus === 'cN2' || data.nodalStatus === 'cN3';
  const regimens: DoseRegimen[] = data.hasM1 ? [{ categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Palyatif', regimenName: 'Palyatif RT (30 Gy)', totalDoseGy: 30, fractionCount: 10, dosePerFractionGy: 3, alphaBeta: 10, bedGy: 39, eqd2Gy: 32.5, targetVolumeCTV: 'Kitle', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: '5mm', ctvToPtvMargin: '5mm', clinicalIndication: 'Palyasyon', preferredBadge: true }] : isN2 ? [{ categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Eşzamanlı KRT', regimenName: 'KRT (60 Gy)', totalDoseGy: 60, fractionCount: 30, dosePerFractionGy: 2, alphaBeta: 10, bedGy: 72, eqd2Gy: 60, targetVolumeCTV: 'Primer+Mediasten', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: '5-7mm', ctvToPtvMargin: '5mm', clinicalIndication: 'Standart', preferredBadge: !data.hasILD }] : [{ categoryRank: '3_ULTRA_HIPOFRAKSIYONE_SBRT', modalityDisplay: 'SBRT', regimenName: 'SBRT (54 Gy / 3 fx)', totalDoseGy: 54, fractionCount: 3, dosePerFractionGy: 18, alphaBeta: 10, bedGy: 151.2, eqd2Gy: 126, targetVolumeCTV: 'ITV', volumeUnionHierarchy: 'PTV=ITV+5mm', gtvToCtvMargin: 'ITV', ctvToPtvMargin: '5mm', clinicalIndication: 'Ablatif', preferredBadge: !data.hasILD }];
...
export function runNasopharynxEngine(data: any): EngineOutput {
  ...
  return { ajccStage: stage, stageSummary: 'Nazofarinks Kanseri', color: data.hasM1 ? 'bg-purple-50 text-purple-900 border-purple-300' : 'bg-rose-50 text-rose-900 border-rose-300', strategy: 'İndüksiyon KT -> Eşzamanlı KRT.', systemic: 'Gemsitabin/Sisplatin.', coreImpact: data.smokingHistory > 0 ? `Sigara: ${data.smokingHistory} paket/yıl.` : '', regimens, url: 'https://www.nccn.org', ref: 'NCCN H&N' };
}
...
export function runHypopharynxEngine(data: any): EngineOutput {
  ...
  return { ajccStage: stage, stageSummary: 'Hipofarinks Kanseri', color: data.hasM1 ? 'bg-purple-50 text-purple-900 border-purple-300' : 'bg-amber-50 text-amber-800 border-amber-300', strategy: 'Eşzamanlı KRT.', systemic: 'Sisplatin.', coreImpact: data.smokingHistory > 0 ? `Sigara: ${data.smokingHistory} paket/yıl.` : '', regimens, url: 'https://www.nccn.org', ref: 'NCCN H&N' };
}
...
export function runEndometriumEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = isHigh ? [{ categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Pelvik EBRT', regimenName: 'Pelvik RT (45 Gy)', totalDoseGy: 45, fractionCount: 25, dosePerFractionGy: 1.8, alphaBeta: 10, bedGy: 53.1, eqd2Gy: 44.3, targetVolumeCTV: 'Vajen kafı + LN', volumeUnionHierarchy: 'PTV=CTV+5-7mm', gtvToCtvMargin: '7mm', ctvToPtvMargin: '5mm', clinicalIndication: 'Yüksek Risk', preferredBadge: true }] : [{ categoryRank: '4_BOOST_OZEL', modalityDisplay: 'VBT', regimenName: 'Vajinal Kaf Brakiterapisi (21 Gy)', totalDoseGy: 21, fractionCount: 3, dosePerFractionGy: 7, alphaBeta: 10, bedGy: 35.7, eqd2Gy: 29.8, targetVolumeCTV: 'Vajen üst 1/3', volumeUnionHierarchy: '0mm', gtvToCtvMargin: '0mm', ctvToPtvMargin: '0mm', clinicalIndication: 'HIR (PORTEC-2)', preferredBadge: true }];
...
export function runBreastEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = [
    { categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Ultra-Hipofraksiyone', regimenName: 'FAST-Forward (26 Gy / 5 fx)', totalDoseGy: 26.0, fractionCount: 5, dosePerFractionGy: 5.2, alphaBeta: 4.0, bedGy: 59.8, eqd2Gy: 39.9, targetVolumeCTV: 'Tüm Meme', volumeUnionHierarchy: 'PTV = CTV + 5 mm', gtvToCtvMargin: 'Glanduler doku', ctvToPtvMargin: '5 mm', clinicalIndication: '1 haftalık standart WBI', preferredBadge: data.positiveNodes == 0 },
    { categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Orta-Hipofraksiyone', regimenName: 'START-B (40.05 Gy / 15 fx)', totalDoseGy: 40.05, fractionCount: 15, dosePerFractionGy: 2.67, alphaBeta: 4.0, bedGy: 66.8, eqd2Gy: 44.5, targetVolumeCTV: `Meme/Göğüs Duvarı ± ${rni}`, volumeUnionHierarchy: 'PTV = CTV + 5 mm', gtvToCtvMargin: 'ESTRO sınırları', ctvToPtvMargin: '5 mm', clinicalIndication: 'RNI/PMRT standardı', preferredBadge: data.positiveNodes > 0 }
  ];
...
export function runGliomaEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = isGrade4 ? (isElderly ? [{ categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Hipofraksiyone', regimenName: 'Yaşlı Şeması (40.05 Gy)', totalDoseGy: 40.05, fractionCount: 15, dosePerFractionGy: 2.67, alphaBeta: 10, bedGy: 50.7, eqd2Gy: 42.3, targetVolumeCTV: 'Kavite+Rezidü', volumeUnionHierarchy: 'PTV=CTV+3mm', gtvToCtvMargin: '15mm', ctvToPtvMargin: '3mm', clinicalIndication: 'Kırılgan', preferredBadge: true }] : [{ categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Stupp', regimenName: 'Stupp (60 Gy / 30 fx)', totalDoseGy: 60, fractionCount: 30, dosePerFractionGy: 2, alphaBeta: 10, bedGy: 72, eqd2Gy: 60, targetVolumeCTV: 'Kavite+Ödem', volumeUnionHierarchy: 'PTV=CTV+3-5mm', gtvToCtvMargin: '20mm', ctvToPtvMargin: '3mm', clinicalIndication: 'Standart GBM', preferredBadge: true }]) : [{ categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'LGG', regimenName: 'LGG (50.4-54 Gy)', totalDoseGy: 50.4, fractionCount: 28, dosePerFractionGy: 1.8, alphaBeta: 10, bedGy: 59.5, eqd2Gy: 49.6, targetVolumeCTV: 'FLAIR', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: '15mm', ctvToPtvMargin: '5mm', clinicalIndication: 'LGG Adjuvan', preferredBadge: true }];
  let core = '';
  if (data.seizures) core += 'Nöbet öyküsü var (Antiepileptik optimizasyonu). ';
  if (data.neuroDeficit) core += 'Nörolojik defisit mevcut. ';
  return { ajccStage: isGrade4 ? 'WHO Grade 4 (GBM)' : 'WHO Grade 2-3', stageSummary: data.severeMassEffect ? 'ACİL DEKOMPRESYON GEREKLİ' : isGrade4 ? 'Glioblastoma' : 'Düşük Dereceli Gliom', color: data.severeMassEffect ? 'bg-rose-100 text-rose-950 font-bold border-rose-500' : isGrade4 ? 'bg-purple-50 text-purple-900 border-purple-300' : 'bg-blue-50 text-blue-800 border-blue-300', strategy: data.severeMassEffect ? '⚠️ CİDDİ KİTLE ETKİSİ / ŞİFT: RT kontrendikedir, acil nöroşirürjikal dekompresyon ve anti-ödem tedavi şarttır.' : isGrade4 ? 'RT + TMZ.' : 'RT + PCV/TMZ.', systemic: 'Temozolomid. Deksametazon taper.', coreImpact: core, regimens: data.severeMassEffect ? [] : regimens, url: 'https://www.eano.eu', ref: 'EANO' };
}
...
export function runMeningiomaEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = canDoSRS ? [{ categoryRank: '3_ULTRA_HIPOFRAKSIYONE_SBRT', modalityDisplay: 'SRS', regimenName: 'Tek Seans SRS (12-14 Gy)', totalDoseGy: 14, fractionCount: 1, dosePerFractionGy: 14, alphaBeta: 3, bedGy: 79.3, eqd2Gy: 47.6, targetVolumeCTV: 'Tümör', volumeUnionHierarchy: 'PTV=CTV+1mm', gtvToCtvMargin: '0mm', ctvToPtvMargin: '1mm', clinicalIndication: 'Küçük benign', preferredBadge: true }] : [{ categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Fraksiyone IMRT', regimenName: `IMRT (${data.whoGrade === 1 ? '54 Gy' : '59.4-66 Gy'})`, totalDoseGy: data.whoGrade === 1 ? 54 : 60, fractionCount: data.whoGrade === 1 ? 30 : 33, dosePerFractionGy: 1.8, alphaBeta: 3, bedGy: 86.4, eqd2Gy: 51.8, targetVolumeCTV: 'GTV + dural kuyruk', volumeUnionHierarchy: 'PTV=CTV+2-3mm', gtvToCtvMargin: '5mm', ctvToPtvMargin: '2-3mm', clinicalIndication: 'Standart Fraksiyone', preferredBadge: true }];
  let core = '';
  if (data.seizures) core += 'Nöbet öyküsü var. ';
  if (data.neuroDeficit) core += 'Nörolojik defisit mevcut. ';
  return { ajccStage: `Grade ${data.whoGrade}`, stageSummary: data.severeMassEffect ? 'ACİL DEKOMPRESYON GEREKLİ' : 'Menenjiom', color: data.severeMassEffect ? 'bg-rose-100 text-rose-950 font-bold border-rose-500' : 'bg-emerald-50 text-emerald-800 border-emerald-300', strategy: data.severeMassEffect ? '⚠️ CİDDİ KİTLE ETKİSİ: SRS mutlak kontrendike. Açık cerrahi dekompresyon gerekir.' : canDoSRS ? 'SRS.' : 'Fraksiyone RT.', systemic: 'Yok.', coreImpact: core, regimens: data.severeMassEffect ? [] : regimens, url: 'https://www.eano.eu', ref: 'EANO' };
}
...
export function runSarcomaEngine(data: any): EngineOutput {
  ...
  const regimens: DoseRegimen[] = isPreop ? [
    { categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Preop RT', regimenName: 'Preoperatif STS (50 Gy / 25 fx)', totalDoseGy: 50.0, fractionCount: 25, dosePerFractionGy: 2.0, alphaBeta: 4.0, bedGy: 75.0, eqd2Gy: 50.0, targetVolumeCTV: 'GTV + Longitudinal Marj (Boyuna 3-4 cm, radyal 1.5 cm)', volumeUnionHierarchy: 'PTV = CTV + 5-7 mm', gtvToCtvMargin: 'Boyuna 30-40 mm, radyal 15 mm', ctvToPtvMargin: '5-7 mm', clinicalIndication: 'Ekstremite yumuşak doku sarkomunda cerrahi öncesi standart shrink-RT', preferredBadge: true }
  ] : [
    { categoryRank: '1_KONVANSIYONEL', modalityDisplay: 'Postop RT', regimenName: 'Postoperatif STS (60-66 Gy)', totalDoseGy: 60.0, fractionCount: 30, dosePerFractionGy: 2.0, alphaBeta: 4.0, bedGy: 90.0, eqd2Gy: 60.0, targetVolumeCTV: 'Cerrahi Yatak + Marjlar', volumeUnionHierarchy: 'PTV = CTV + 7 mm', gtvToCtvMargin: 'Boyuna 30 mm, radyal 15 mm', ctvToPtvMargin: '7 mm', clinicalIndication: 'Cerrahi sınır pozitifliği (R1) veya yüksek grade olgularda', preferredBadge: true }
  ];
  return { 
    ajccStage: data.hasM1 ? 'Evre IV' : data.depth === 'DEEP' ? 'Evre III (Derin Kompartman)' : 'Evre I/II (Yüzeyel)', 
    stageSummary: data.depth === 'DEEP' ? 'Derin Yerleşimli Yüksek Grade STS' : 'Yüzeyel Sarkom', 
    color: 'bg-indigo-50 text-indigo-900 border-indigo-300', 
    strategy: isPreop ? 'Preoperatif 50 Gy (Akut yara komplikasyonu riski vardır ancak geç fibrozis daha azdır).' : 'Postoperatif 60-66 Gy.', 
    systemic: 'Doxorubicin + Ifosfamid (Seçilmiş yüksek grade/metastatik olgularda).', 
    coreImpact: 'FNCLC grade ve derinlik evrelemede en kritik prognostik faktörlerdir.',
    regimens, 
    url: 'https://www.nccn.org', 
    ref: 'NCCN Soft Tissue Sarcoma v1.2026' 
  };
}

export function runMelanomaEngine(data: any): EngineOutput {
  const regimens: DoseRegimen[] = data.setting === 'BRAIN_METASTASIS' ? [{ categoryRank: '3_ULTRA_HIPOFRAKSIYONE_SBRT', modalityDisplay: 'SRS', regimenName: 'Beyin Met SRS (18-24 Gy)', totalDoseGy: 21.0, fractionCount: 1, dosePerFractionGy: 21.0, alphaBeta: 2.5, bedGy: 197.4, eqd2Gy: 109.7, targetVolumeCTV: 'GTV', volumeUnionHierarchy: 'PTV=CTV+1mm', gtvToCtvMargin: '0mm', ctvToPtvMargin: '1mm', clinicalIndication: 'Ablatif', preferredBadge: true }] : [{ categoryRank: '2_HIPOFRAKSIYONE', modalityDisplay: 'Adjuvan', regimenName: 'Adjuvan Nodal RT (48 Gy / 20 fx)', totalDoseGy: 48.0, fractionCount: 20, dosePerFractionGy: 2.4, alphaBeta: 2.5, bedGy: 94.1, eqd2Gy: 52.3, targetVolumeCTV: 'Lenf Nodu Havzası', volumeUnionHierarchy: 'PTV=CTV+5mm', gtvToCtvMargin: 'Cerrahi yatak', ctvToPtvMargin: '5mm', clinicalIndication: 'Nodal nüks önleme', preferredBadge: true }];
...
export function runBonePalliativeEngine(data: any): EngineOutput {
  ...
  return { ajccStage: 'Palyatif M1', stageSummary: lifeEx === '<3' ? 'Kısa Yaşam Beklentisi' : isSbrt ? 'Oligometastaz' : isCordComp ? 'Kord Basısı' : 'Kemik Ağrısı', color: isCordComp ? 'bg-rose-100 text-rose-950 font-bold' : lifeEx === '<3' ? 'bg-slate-200 text-slate-800' : 'bg-amber-50 text-amber-900 border-amber-300', strategy: lifeEx === '<3' ? 'SADECE 8 Gy Tek Fraksiyon önerilir (Beklenen ömür < 3 ay).' : isSbrt ? 'SBRT.' : isCordComp ? 'Acil Dekompresyon RT.' : '8 Gy Tek Fraksiyon.', systemic: 'Zoledronik Asit.', regimens, url: 'https://www.astro.org', ref: 'ASTRO Bone' };
}